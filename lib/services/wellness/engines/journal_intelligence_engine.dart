...
class JournalIntelligenceEngine {
  ...
  JournalInsight analyzeEntry(JournalEntry entry) {
    ...
    final primaryEmotion =
        sortedEmotions.isNotEmpty ? sortedEmotions.first.key : 'neutral';
    ...
    final secondaryEmotions =
        sortedEmotions.skip(1).take(3).map((e) => e.key).toList();
    ...
    final growthScore = _computeGrowthScore(
        entry.mood, sentimentScore, themes.length, distortions.length);
  ...
  WeeklySummary generateWeeklySummary(List<JournalEntry> entries) {
    ...
    final sortedTopics = topicCount.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    final topTopics = sortedTopics
        .map((e) => TopicFrequency(
              topic: e.key,
              count: e.value,
    ...
    final avgSentiment =
        insights.fold(0.0, (a, i) => a + i.emotions.sentimentScore) /
            insights.length;

    final avgGrowth =
        insights.fold(0.0, (a, i) => a + i.growthScore) / insights.length;
    final firstGrowth = insights.isNotEmpty ? insights.first.growthScore : 50.0;
    final growthTrend = avgGrowth > firstGrowth
        ? 'improving'
        : avgGrowth < firstGrowth
            ? 'declining'
            : 'stable';
  ...
  MonthlySummary generateMonthlySummary(List<WeeklySummary> weeks) {
    ...
    final avgMood = weeks.fold(0.0, (a, w) => a + w.averageMood) /
        (weeks.length).clamp(1, 999);
  ...
  List<String> _extractGratitude(String content) {
    ...
    if (lower.contains('grateful for') || lower.contains('thankful for')) {
      ...
      for (final s in sentences) {
        if (s.toLowerCase().contains('grateful') ||
            s.toLowerCase().contains('thankful')) {
          items.add(s.trim());
        }
      }
    }
  ...
  List<CognitiveDistortion> _detectDistortions(String lower, String original) {
    ...
    if (RegExp(r'\b(always|never|everyone|no one|everything|nothing)\b')
        .hasMatch(lower)) {
      final match =
          RegExp(r'\b(always|never|everyone|no one|everything|nothing)\b')
              .stringMatch(lower);
    ...
    if (RegExp(r'\b(terrible|awful|horrible|disaster|catastrophe)\b')
        .hasMatch(lower)) {
      distortions.add(const CognitiveDistortion(
    ...
    if (RegExp(r"\b(can't|won't|cannot|impossible|hopeless)\b")
        .hasMatch(lower)) {
      distortions.add(const CognitiveDistortion(
    ...
    if (RegExp(
            r"\b(my fault|my mistake|i messed up|i failed|i'm not good enough)\b")
        .hasMatch(lower)) {
      distortions.add(const CognitiveDistortion(
  ...
  List<String> _extractPositiveMoments(String original, String lower) {
    ...
    for (final s in sentences) {
      ...
      final positiveWordCount =
          _positiveWords.where((w) => sl.contains(w)).length;
  ...
  double _computeGrowthScore(
      int mood, double sentiment, int themesCount, int distortionCount) {
    ...
    return (moodScore + sentimentScore + awarenessScore + challengeScore)
        .clamp(0, 100);
  }
  ...
  double _awarenessScore(List<JournalInsight> insights) {
    final avgThemes =
        insights.fold(0.0, (a, i) => a + i.dominantThemes.length) /
            insights.length;
  ...
  double _resilienceScore(List<JournalInsight> insights) {
    final positiveEndings =
        insights.where((i) => i.emotions.sentimentScore >= 0).length;
    return (positiveEndings / insights.length * 100).roundToDouble();
  }

  double _selfCompassionScore(List<JournalInsight> insights) {
    final withGratitude =
        insights.where((i) => i.gratitudeItems.isNotEmpty).length;
  ...
  List<String> _generateWeeklyInsights(List<JournalInsight> insights) {
    ...
    if (avgMood >= 4.0) {
      ...
      list.add(
          'Your mood has been moderate this week with room for improvement.');
    }
    ...
    if (distortions.isNotEmpty) {
      list.add(
          '${distortions.length} cognitive distortion patterns detected this week.');
    }
    return list;
  }
  ...
  String _generateMonthlyNarrative(List<WeeklySummary> weeks) {
    ...
    final totalEntries = weeks.fold(0, (a, w) => a + w.entryCount);
    final trend =
        weeks.length >= 2 && weeks.last.averageMood > weeks.first.averageMood
            ? 'improving'
            : 'stable';
