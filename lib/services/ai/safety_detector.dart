class SafetyAssessment {
  ...
  factory SafetyAssessment.fromJson(Map<String, dynamic> json) => SafetyAssessment(
    ...
    requiresImmediateEscalation: json['requiresImmediateEscalation'] ?? false,
  );
}
...
class SafetyDetector {
  ...
  static final List<_CrisisPattern> _patterns = [
    _CrisisPattern(r'\b(kill myself|end my life|commit suicide|want to die)\b', 'suicidal', 0.95, true),
    _CrisisPattern(r'\b(self.?harm|cut myself|hurt myself|burn myself)\b', 'self_harm', 0.90, true),
    _CrisisPattern(r"\b(no reason to live|better off dead|can't go on)\b", 'suicidal', 0.85, true),
    _CrisisPattern(r'\b(ending it all|final goodbye|last goodbye)\b', 'suicidal', 0.90, true),
    _CrisisPattern(r'\b(hopeless|no hope|nothing matters anymore)\b', 'hopelessness', 0.60, false),
    _CrisisPattern(r'\b(abused|being abused|physical abuse|sexual abuse)\b', 'abuse', 0.70, true),
    _CrisisPattern(r'\b(emergency|i need help now|help me please)\b', 'emergency', 0.75, true),
    _CrisisPattern(r"\b(can't take it anymore|i give up|i quit)\b", 'hopelessness', 0.65, false),
    _CrisisPattern(r'\b(no one cares|no one understands|all alone)\b', 'loneliness', 0.40, false),
    _CrisisPattern(r'\b(pills? to end it| overdose|hanging|jump off)\b', 'suicidal', 0.95, true),
  ];

  static SafetyAssessment assess(String text) {
    if (text.isEmpty) return const SafetyAssessment();
...
class _CrisisPattern {
  ...
  _CrisisPattern(String pattern, this.type, this.confidence, this.requiresEscalation)
