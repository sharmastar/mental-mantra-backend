...
class _ForgotPasswordPageState extends ConsumerState<ForgotPasswordPage>
  ...
  Future<void> _sendReset() async {
    ...
    try {
      final success = await ref.read(authStateProvider.notifier).forgotPassword(_emailCtrl.text.trim());
      if (mounted) {
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(error ?? 'Failed to send reset email. Please try again.'), backgroundColor: Colors.red),
          );
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final isLoading = _isSending || ref.watch(authStateProvider.select((s) => s.isLoading));
          ...
          child: FadeTransition(
            ...
            child: SlideTransition(
              ...
              child: Padding(
                ...
                child: _emailSent ? _buildSuccess(isDark) : _buildForm(context, isLoading, isDark),
              ),
            ),
          ),
        ),
        ...
        children: [
          ...
          Center(
            child: Container(
              ...
              child: const Icon(Icons.lock_reset_rounded, size: 38, color: Colors.white),
            ),
          ),
        ...
        children: [
          ...
          Text(
            ...
            style: GoogleFonts.playfairDisplay(
              fontSize: 28,
              fontWeight: FontWeight.w700,
              color: isDark ? const Color(0xFFE2F3F2) : const Color(0xFF092828),
            ),
          ),
          const SizedBox(height: 16),
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: 15,
                color: isDark ? const Color(0xFF6B8A88) : const Color(0xFF9EAEAD),
                height: 1.6,
              ),
            ),
          ),
