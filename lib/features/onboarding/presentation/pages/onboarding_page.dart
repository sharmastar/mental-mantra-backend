...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  int _step = 0; // 0: Welcome/Consent, 1: Assessment, 2: Analysis, 3: Completion
  String _userName = '';
  ...
  void _onAssessmentComplete(List<AssessmentResponse> responses) {
    try {
      ...
      for (final response in responses) {
        ...
        if (response.questionId == 'nickname' && response.answer != null && (response.answer as String).isNotEmpty) {
          nickname = response.answer as String;
        } else if (response.questionId == 'age_group' && response.answer != null) {
      ...
      final Future<void> processFuture = () async {
        ...
        final classification = <String, dynamic>{
          'primaryDomain': sorted.isNotEmpty ? domainKey(sorted.first.key) : 'stress_burnout',
          'secondaryDomains': sorted.length > 1 ? [domainKey(sorted[1].key)] : <String>[],
          'scores': domainScores.map((k, v) => MapEntry(domainKey(k), v.toDouble())),
          'confidence': 0.85,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to submit assessment: $e'), backgroundColor: Colors.redAccent),
        );
      }
    }
  }
  ...
  void _onAnalysisComplete() {
    ...
    setState(() {
      _profile = profileState ?? {
        'overallScore': 75.0,
        'summary': 'Your wellness profile shows good resilience with opportunities to improve sleep and handle stress.',
        'recommendedFocusAreas': ['Sleep', 'Anxiety management', 'Physical health'],
      };
      ...
      body: SafeArea(
          ...
          children: [
            SingleChildScrollView(
              ...
              Text(
                ...
                style: GoogleFonts.outfit(fontSize: 16, color: purpleTheme, fontWeight: FontWeight.w500),
              ),
              const SizedBox(height: 24),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: purpleTheme.withValues(alpha: 0.12)),
                ),
                child: Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.spa_rounded, color: purpleTheme, size: 20),
                    ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        const Icon(Icons.tune_rounded, size: 18, color: purpleTheme),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
                            'Your responses help us personalize your experience and recommendations.',
                            style: GoogleFonts.outfit(fontSize: 13, color: titleColor, height: 1.4),
                          ),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        const Icon(Icons.skip_next_rounded, size: 18, color: purpleTheme),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
              ...
              _buildConsentTile(
                ...
                onChanged: (val) => setState(() => _consent1 = val ?? false),
              ),
              const SizedBox(height: 12),
              _buildConsentTile(
                value: _consent2,
                label: 'Do you agree to the Terms & Conditions?',
                onChanged: (val) => setState(() => _consent2 = val ?? false),
              ),
              const SizedBox(height: 12),
              _buildConsentTile(
                value: _consent3,
                label: 'I understand that Mental Mantra is a wellness-support platform and not a substitute for medical diagnosis or emergency psychiatric care.',
                onChanged: (val) => setState(() => _consent3 = val ?? false),
              ),
              const SizedBox(height: 12),
              _buildConsentTile(
                value: _consent4,
                label: 'Are you ready to begin your personalized assessment?',
                onChanged: (val) => setState(() => _consent4 = val ?? false),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text('Let\'s Get Started', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                      const SizedBox(width: 8),
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: IconButton(
              icon: Icon(
                isMusicPlaying ? Icons.music_note : Icons.music_note_outlined,
                color: isMusicPlaying ? purpleTheme : Colors.grey,
              ),
              onPressed: () => ref.read(backgroundMusicProvider.notifier).toggle(),
              tooltip: isMusicPlaying ? 'Mute Background Music' : 'Play Background Music',
            ),
          ),
        ),
      ],
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: value ? purpleTheme : const Color(0xFFE5E7EB)),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                label,
                style: GoogleFonts.outfit(fontSize: 14, color: titleColor, fontWeight: FontWeight.w600),
              ),
            ),
          ],
        ),
