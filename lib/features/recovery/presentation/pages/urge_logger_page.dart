  ...
  Future<void> _save() async {
    if (_triggerController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Please describe what triggered the urge')),
      );
      return;
    }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_resisted
              ? 'Urge resisted! Stay strong'
              : 'Logged. Next time you will resist'),
          backgroundColor:
              _resisted ? AppTheme.successColor : AppTheme.warningColor,
        ),
      );
      context.pop();
    }
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: UrgeType.values
                  .map((type) => ChoiceChip(
                        label: Text(_urgeTypeLabel(type)),
                        selected: _selectedUrgeType == type,
                        onSelected: (_) =>
                            setState(() => _selectedUrgeType = type),
                        selectedColor:
                            _urgeTypeColor(type).withValues(alpha: 0.2),
                      ))
                  .toList(),
            ),
            ...
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: UrgeLog.copingStrategies
                  .map((s) => FilterChip(
                        label: Text(s['label'] as String,
                            style: const TextStyle(fontSize: 12)),
                        selected: _selectedCoping == s['id'],
                        onSelected: (_) => setState(() => _selectedCoping =
                            _selectedCoping == s['id']
                                ? null
                                : s['id'] as String),
                        selectedColor:
