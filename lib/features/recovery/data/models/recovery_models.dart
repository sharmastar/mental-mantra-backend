...
enum UrgeType { gaming, socialMedia, appBrowsing, videoStreaming, shopping, other }

enum DetoxSessionType { focus, digitalDetox, gamingBreak, socialMediaFree }

enum RecoveryTargetType { screenTime, gamingHours, appUsage, abstinenceDays, detoxSessions }
...
class UrgeLog {
  ...
  static const List<Map<String, dynamic>> copingStrategies = [
    {'id': 'urge_surfing', 'label': 'Urge Surfing Meditation', 'icon': Icons.waves},
    {'id': 'deep_breathing', 'label': 'Deep Breathing (4-7-8)', 'icon': Icons.air},
    {'id': 'physical_activity', 'label': 'Walk or Exercise', 'icon': Icons.directions_walk},
    {'id': 'journal', 'label': 'Write About the Urge', 'icon': Icons.edit_note},
    {'id': 'cold_water', 'label': 'Drink Cold Water', 'icon': Icons.water_drop},
    {'id': 'music', 'label': 'Listen to Calming Music', 'icon': Icons.music_note},
    {'id': 'talk', 'label': 'Call or Text Someone', 'icon': Icons.phone},
    {'id': 'meditation', 'label': 'Quick Guided Meditation', 'icon': Icons.self_improvement},
    ...
    {'id': 'affirmation', 'label': 'Read Recovery Affirmations', 'icon': Icons.auto_awesome},
  ];
}
...
class DetoxSession {
  ...
  int get progressPercent => durationMin > 0 ? (completedMinutes * 100 / durationMin).round().clamp(0, 100) : 0;
...
class RecoveryGoal {
  ...
  int get progressPercent => targetValue > 0 ? (currentValue * 100 ~/ targetValue).clamp(0, 100) : 0;
...
class RecoveryStats {
  ...
  double get resistanceRate => totalUrgesLogged > 0 ? urgesResisted / totalUrgesLogged : 0;
